...
# corrP = cor(residuals(lm(as.matrix(growthF6[,mice_growth$traits])~growthF6$Sex)))
# gcorr_M = cor(growthF6[growthF6$Sex == "M",mice_growth$traits])
# gcorr_F = cor(growthF6[growthF6$Sex == "F",mice_growth$traits])
# plot_grid(superheat(gcorr_M), superheat(gcorr_F))
# colnames(gcorr) = gsub("growth_", "", mice_growth$traits)
...
mean_growth_strain = ddply(mice_growth$Strain, .(Strain), numcolwise(mean))
mean_growth_F6 = ddply(mice_growth$F6, .(Litter), numcolwise(mean))

g_means = colMeans(mean_growth_strain[,mice_growth$traits])
m_growth_strain = gather(mean_growth_strain, period, growth, growth_D0D3:growth_D49D56)
m_growth_f6 = gather(mice_growth$F6, period, growth, growth_D0D3:growth_D49D56)
m_growth_strain$growth_std = m_growth_strain$growth/g_means[m_growth_strain$period]

m_growth_strain$period = factor(m_growth_strain$period, levels = mice_growth$traits)
stain_growth = ggplot(m_growth_strain, aes(period, growth, group = Strain, color = Strain)) + geom_line(size = 1) + 
  scale_x_discrete(labels = gsub("growth_", "", mice_growth$traits)) + labs(y = "Growth per interval (g)", x = "Intervals") + 
  geom_abline(slope = 0) +
...
stain_growth_std = ggplot(m_growth_strain, aes(period, growth_std, group = Strain, color = Strain)) + geom_line(size = 1) + 
  scale_x_discrete(labels = gsub("growth_", "", mice_growth$traits)) + labs(y = "Mean scaled growth (g)", x = "Intervals") + 
...
m_growth_f6$period = factor(m_growth_f6$period, levels = mice_growth$traits)
